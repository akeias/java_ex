...
class GStack<T>{ // 제네릭 스택 선언, 타입은 T
	int tos;
	...
	public void push(T item) {
		if(tos == 10)// 스택이  꽉 촤서 더이상 요소를 삽입 할 수 없음.
			return;
		stck[tos] = item;
		tos++;
	}
	public T pop() {
		if(tos == 0) // 스택이 비어 있어 꺼낼 요소가 없음.
			return null;
		tos--;
		return (T)stck[tos]; // 타입 매개변수 타입으로 ㅋ스팅
	}
}
...
public class ex7_9 {
	...
	public static void main(String[] args) {
		GStack<String> stringStack = new GStack<String>(); //Stirng 타입의 GStack 생성
